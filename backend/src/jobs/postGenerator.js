...
async function generateRandomPost() {
    ...
    try {
        ...
        if (users.length === 0) {
            console.log(
                '⚠️ Aucun utilisateur trouvé, impossible de générer un post',
            );
            return null;
        }
      ...
      ORDER BY c.created_at DESC`,
        ...
        try {
            ...
            console.log(
                `📡 Notification Socket.IO envoyée pour le post #${postId}`,
            );
        } catch (error) {
            console.log(
                `⚠️ Socket.IO pas encore initialisé, notification ignorée : ${error.message}`,
            );
        }
